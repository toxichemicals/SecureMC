...
client:
	...
	CGO_ENABLED=1 GOOS=linux GOARCH=amd64 go build -o $(CBUILD_DIR)/client-$(CLIENTNAME)-Linux-amd64 client.go
	# Windows build (Arch MinGW-w64 cross-compiler)
	CGO_ENABLED=1 CC=x86_64-w64-mingw32-gcc GOOS=windows GOARCH=amd64 go build -ldflags "-H=windowsgui" -o $(CBUILD_DIR)/client-$(CLIENTNAME)-Windows-amd64.exe client.go
